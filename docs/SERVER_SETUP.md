...
```bash
curl -fsSL https://raw.githubusercontent.com/reaper1067MSX/MAGI/main/install.sh | bash -s -- --agent gemini
```
...
pipeline {
    ...
    stages {
        stage('Install MAGI') {
            steps {
                sh 'curl -fsSL https://raw.githubusercontent.com/reaper1067MSX/MAGI/main/install.sh | bash -s -- --agent gemini'
            }
        }
...
# Install MAGI
RUN git clone https://github.com/reaper1067MSX/MAGI.git /opt/MAGI
WORKDIR /opt/MAGI
